...
export default function ColdCallsPage() {
  ...
  // Get unevaluated calls that have transcript
  const unevaluatedCalls = useMemo(() => {
    return calledCalls.filter(c => 
      (c.transcript_text || c.notes) && 
      c.ai_evaluation_status !== 'completed' && 
      c.ai_evaluation_status !== 'processing' &&
      (c.status === 'completed' || c.status === 'answered')
    );
  }, [calledCalls]);

  const handleBatchEvaluate = async () => {
    if (unevaluatedCalls.length === 0) return;
    setBatchEvaluating(true);
    let success = 0;
    let failed = 0;
    for (const call of unevaluatedCalls) {
      try {
        await evaluateCall.mutateAsync(call.id);
        success++;
      } catch {
        failed++;
      }
    ...
    toast.success(`F-CVS batch complete: ${success} scored, ${failed} failed`);
  };
            ...
            {unevaluatedCalls.length > 0 && (
              ...
              >
                {batchEvaluating ? (
                  ...
                  <><Brain className="w-4 h-4" /> Score {unevaluatedCalls.length} Calls</>
                )}
              </Button>
            )}

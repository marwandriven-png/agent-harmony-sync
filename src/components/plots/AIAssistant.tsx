        ...
        switch (type) {
            ...
            default:
                ...
                if (lowerQuery.includes('villa')) {
                    filtered = plots.filter(p => p.zoning.toLowerCase().includes('villa'));
                    message = `Found ${filtered.length} villa plots matching your criteria.`;
                } else if (lowerQuery.includes('commercial')) {
                    filtered = plots.filter(p => p.zoning.toLowerCase().includes('commercial'));
                    message = `Found ${filtered.length} commercial plots in the development.`;
                } else if (lowerQuery.includes('mixed')) {
                    filtered = plots.filter(p => p.zoning.toLowerCase().includes('mixed'));
                    message = `Found ${filtered.length} mixed-use plots with flexible development options.`;
                } else {
                    message = `Analyzing "${query}"... I found ${plots.length} total plots in the development. Try asking about specific plot types like "villa plots" or "commercial plots".`;
                }
        }
                ...
                <div className="flex gap-2 flex-wrap">
                    {QUICK_PROMPTS.map((prompt) => (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-muted hover:bg-muted/80 rounded-lg text-xs transition-colors"
                        >
                ...
                {response ? (
                    <div className="space-y-4">
                        ...
                        {recommendedPlots.length > 0 && (
                            <div className="space-y-2">
                                <div className="text-xs text-muted-foreground font-semibold px-1">Recommended Plots:</div>
                                {recommendedPlots.map((plot) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-left p-3 rounded-xl border transition-all hover:bg-muted/50 ${selectedPlot?.id === plot.id ? 'border-primary ring-1 ring-primary/20' : 'bg-card'
                                                }`}
                                        >
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="font-bold">{plot.id}</span>
                                                <span className="text-emerald-600 font-semibold text-sm">{feasibility.roi}% ROI</span>
                                            </div>
                    ...
                    <div className="h-full flex items-center justify-center text-center opacity-40">
                        <div>
                            ...
                            <p className="text-sm">Ask me about plots, ROI analysis, or development opportunities</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="p-4 border-t bg-muted/10">
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-1 px-4 py-2 bg-background border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none"
                    />
                    <button

...
class DDAGISService {
  ...
  async fetchAffectionPlan(plotId: string): Promise<AffectionPlanData | null> {
    // Return mock data for demo plots (IDs starting with PA)
    if (plotId.startsWith('PA')) {
      const demoPlots = generateDemoPlots();
      const plot = demoPlots.find(p => p.id === plotId);
      if (plot) {
        return {
          plotNumber: plot.id,
          entityName: plot.entity || 'Dubai South',
          projectName: plot.project || 'Demo Phase',
          landName: 'Standard Plot',
          areaSqm: plot.area,
          gfaSqm: plot.gfa,
          gfaType: 'Residential',
          maxHeightFloors: plot.floors,
          maxHeightMeters: plot.maxHeight || 12,
          maxHeight: `${plot.maxHeight || 12}m`,
          heightCategory: 'Low Rise',
          maxPlotCoverage: plot.plotCoverage || 60,
          minPlotCoverage: 10,
          plotCoverage: `${plot.plotCoverage || 60}%`,
          buildingSetbacks: {
            side1: '3m',
            side2: '3m',
            side3: '5m',
          ...
          mainLanduse: plot.zoning,
          subLanduse: 'Residential',
          landuseDetails: 'Independent or Semi-detached villa',
          landuseCategory: 'Residential',
          generalNotes: 'Demostatration data for HyperPlot AI interface verification.',
          siteplanIssueDate: Date.now() - 30 * 24 * 60 * 60 * 1000,
          siteplanExpiryDate: Date.now() + 335 * 24 * 60 * 60 * 1000,
          siteStatus: plot.status,
          isFrozen: plot.isFrozen,
          freezeReason: plot.freezeReason || null,
        };
      }
    }
